	...
	SubShader
	{
		...
		Pass
		{
			...
			float2 Twirl(float2 uv, float2 center, float2 offset, float strength)
			{
				...
				// x = lerp(x, delta.x, 2 * length(delta) * (1 - _ProgressCtrl));
				// y = lerp(y, delta.y, 2 * length(delta) * (1 - _ProgressCtrl));
			...
			half4 frag(v2f IN) : SV_Target
			{
				...
				float backLine = max(0, ctrl - _DistortUVStart)
					* (SAMPLE_TEXTURE2D(_DistortUVTex, s_linear_clamp_sampler, twirl).rg);
				backCol += backLine;
